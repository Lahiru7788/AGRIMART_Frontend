...
const ProductDetailsPage = () => {
    ...
    const checkPendingOrder = async () => {
        try {
            ...
            const supermarketUserID = getWithExpiry('userID');

            if (!supermarketUserID) {
            ...
            const response = await axios.get(
                `http://localhost:8081/api/user/viewSupermarketOrdersByConsumerID/${supermarketUserID}`,
...
// Updated handleAddToCart function
            ...
            const addToCartPromises = confirmedOrderIds.map(async (orderID) => {
                try {
                    const response = await axios.put(
                        `http://localhost:8081/api/user/Supermarket-addedToCart/${orderID}/confirm`,
    ...
    const handleBackToProducts = () => {
        router.push('/supermarketViewFarmerProducts');// Go back to previous page
    };
