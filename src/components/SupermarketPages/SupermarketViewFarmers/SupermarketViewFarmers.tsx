...
const FarmerCard = ({ farmer }) => {
    ...
    const handleProfileClick = () => {
        router.push(`/supermarketViewFarmerProfile?userID=${farmer.user.userID}`);
    };

    const handleViewProducts = () => {
        router.push(`/supermarketViewFarmerProductDetails?userID=${farmer.user.userID}`);
    };
    ...
    const fetchFarmers = async () => {
        try {
            ...
            const farmersList = userDetailsList.filter(userDetail =>
                userDetail.user && userDetail.user.userType === "Farmer"
            );
