...
const ConsumerOrderHistory = () => {
    ...
    useEffect(() => {
        filteredOrders.forEach(order => {
            if (order.rawOrder?.sfProduct?.user?.userID && !profileImages[order.rawOrder.sfProduct.user.userID]) {
                fetchFarmerProfileImage(order.rawOrder.sfProduct.user.userID);
            }
        });
    }, [filteredOrders]);
    ...
    const fetchOrders = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:8081/api/user/viewSupermarketSeedsOrdersByConsumerID/${userID}`);

            if (response.data && response.data.supermarketSeedsOrderGetResponse) {
                const ordersData = response.data.supermarketSeedsOrderGetResponse;
                ...
                const processedOrders = ordersData
                    ...
                    .map(order => {
                        ...
                        return {
                            ...
                            productID: order.sfProduct?.productID || 0,
                            productName: order.productName,
                            category: order.productCategory || 'N/A',
                            sfSellerName: order.sfProduct?.user ?
                                `${order.sfProduct.user.firstName} ${order.sfProduct.user.lastName}` : 'N/A',
                            farmerEmail: order.sfProduct?.user?.userEmail || 'N/A',
    ...
    const applyFilters = () => {
        ...
        if (filters.sfSellerName) {
            filtered = filtered.filter(order =>
                order.sfSellerName.toLowerCase().includes(filters.sfSellerName.toLowerCase())
            );
        }
    ...
    const handleProfileClick = (order) => {
        if (order.rawOrder && order.rawOrder.sfProduct && order.rawOrder.sfProduct.user && order.rawOrder.sfProduct.user.userID) {
            router.push(`/consumerViewSFProfile?userID=${order.rawOrder.sfProduct.user.userID}`);
        }
    };

    const handleUpdateOrder = (order) => {
        const encodedOrder = encodeURIComponent(JSON.stringify(order.rawOrder));
        router.push(`/supermarketUpdateSFProductDetailsPage?orderDetails=${encodedOrder}&productID=${order.productID}&orderID=${order.orderID}`);
    };
    ...
    const deleteOrder = async () => {
        ...
        try {
            ...
            await axios.put(`http://localhost:8081/api/user/supermarket-seedsOrder-products/${selectedOrder.orderID}/delete`);
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6 sticky top-0 z-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-poppins-bold text-gray-700 mb-2">
                                Seed & Fertilizer Seller Name
                            </label>
                            <input
                                type="text"
                                value={filters.sfSellerName}
                                onChange={(e) => handleFilterChange('sfSellerName', e.target.value)}
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                                <h2 className="text-xl font-poppins-bold text-gray-800">
                                    Supermarket Seeds or Fertilizer Orders ({filteredOrders.length})
                                </h2>
                                        ...
                                        <tr key={order.id} className={`hover:bg-gray-50 transition-colors border-l-4 ${getStatusColor(order.status)}`}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    ...
                                                    >
                                                        {profileImages[order.rawOrder?.sfProduct?.user?.userID] ? (
                                                            <img
                                                                src={profileImages[order.rawOrder.sfProduct.user.userID]}
                                                                alt="Profile"
                    ...
                    <div className="bg-white rounded-xl p-6 max-w-md w-full mx-4">
                        ...
                        <div className="bg-gray-50 rounded-lg p-3 mb-6">
                            <div className="text-sm text-gray-600 font-poppins-regular">
                                <strong>Farmer:</strong> {selectedOrder.sfSellerName}<br />
