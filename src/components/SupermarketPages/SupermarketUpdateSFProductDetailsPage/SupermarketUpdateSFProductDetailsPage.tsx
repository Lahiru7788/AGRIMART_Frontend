...
const ProductDetailsPage = () => {
    ...
    const checkPendingOrder = async () => {
        try {
            ...
            const response = await axios.get(
                `http://localhost:8081/api/user/viewSupermarketSeedsOrdersByConsumerID/${consumerUserID}`,
                { withCredentials: true }
            );

            if (response.status === 200 && response.data.supermarketSeedsOrderGetResponse) {
                const allOrders = response.data.supermarketSeedsOrderGetResponse;
    ...
    const handleUpdateOrder = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                'http://localhost:8081/api/user/supermarketSeedsOrderProducts',
                orderData,
                {
                    headers: {
    ...
    return (
        <div className="container mx-auto p-4 mt-[80px] w-[1060px]">
            ...
            <div className="bg-[#88C34E] rounded-t-xl p-4 mb-0">
                <h1 className="text-white font-poppins-bold text-2xl text-center">Update Supermarket Seeds or Fertilizer Order</h1>
            </div>
