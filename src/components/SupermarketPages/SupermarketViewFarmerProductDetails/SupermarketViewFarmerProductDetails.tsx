...
import ProductDetailsPage from "../SupermarketViewProductDetailsPage/SupermarketViewProductDetailsPage";
...
const ProductCard = ({ product, onViewDetails }) => {
    ...
    const OnViewDetails = () => {
        if (product.user && product.user.userID) {
            // Use query parameter instead of path parameter
            router.push(`/consumerViewProductDetailsPage?productID=${product.productID}`);
        }
    };
...
const ProductList = () => {
    ...
    useEffect(() => {
        ...
        if (userID) {
            ...
            toast.error("User ID is required to view products.", {
                position: "top-right",
                autoClose: 5000,
            });
        }
    ...
    const fetchProducts = async () => {
        try {
            // Use the updated API endpoint with userID from URL parameters
            const response = await axios.get(
        ...
        return (
            <div className="pt-[100px] p-6 bg-gray-100 min-h-screen flex items-center justify-center">
                <div className="text-center">
                    <p className="text-gray-500 font-poppins-regular text-lg">
                        User ID is required to view products.
                    </p>
                </div>
            </div>
        );
