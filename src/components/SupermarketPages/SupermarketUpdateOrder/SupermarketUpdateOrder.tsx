...
const OrderCard = ({ order, onUpdateClick }) => {
    ...
    const handleDelete = async () => {
        try {
            await axios.put(
                `http://localhost:8081/api/user/Supermarket-order/${order.orderID}/delete`,
                {},
    ...
    const fetchOfferDetails = async () => {
        ...
        try {
            const response = await axios.get(
                `http://localhost:8081/api/user/viewSupermarketOffers/${order.orderID}`
            );

            // Access the correct part of the response structure
            if (response.data.supermarketOfferGetResponse &&
                response.data.supermarketOfferGetResponse.length > 0) {
                setOfferDetails(response.data.supermarketOfferGetResponse[0]);
            }
        } catch (error) {
...
const UpdateOrderForm = ({ order, onCancel, onSubmit }) => {
    ...
    const fetchOfferDetails = async () => {
        try {
            ...
            if (response.data.supermarketOfferGetResponse &&
                response.data.supermarketOfferGetResponse.length > 0) {
                const offerDetails = response.data.supermarketOfferGetResponse[0];
                setOfferFormData({
    ...
    const handleSubmitOrderUpdate = async () => {
        try {
            ...
            await axios.post(
                `http://localhost:8081/api/user/supermarketAddOrders`,
                updatedData,
                { withCredentials: true }
            );
    ...
    const handleImageUpdate = async () => {
        ...
        try {
            ...
            await axios.post(
                `http://localhost:8081/api/user/sAddOrderProductImage`,
                imageFormData,
                {
                    headers: {
    ...
    const handleSubmitOfferUpdate = async () => {
        try {
            ...
            const endpoint = order.hasOffer ?
                `http://localhost:8081/api/user/supermarketOffers` :
                `http://localhost:8081/api/user/supermarketOffers`;
...
const ConsumerOrderList = () => {
    ...
    const fetchOrders = async () => {
        try {
            ...
            const response = await axios.get(
                `http://localhost:8081/api/user/viewSupermarketAddOrders/${userID}`
            );

            const orderList = response.data.supermarketAddOrderGetResponse || [];
                ...
                activeOrders.map(async (order) => {
                    ...
                    try {
                        ...
                        updatedOrder.hasOffer =
                            offerResponse.data.supermarketOfferGetResponse &&
                            offerResponse.data.supermarketOfferGetResponse.length > 0;
                    } catch (error) {
                    ...
                    if (order.orderID) {
                        try {
                            const imageResponse = await axios.get(
                                `http://localhost:8081/api/user/viewSupermarketAddOrderImage?orderID=${order.orderID}`,
                                { responseType: "blob" }
                            );
