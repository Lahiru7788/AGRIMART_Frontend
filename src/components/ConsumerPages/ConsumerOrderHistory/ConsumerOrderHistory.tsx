...
const ConsumerOrderHistory = () => {
    ...
    const fetchOrders = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost:8081/api/user/viewConsumerOrdersByConsumerID/${userID}`);

            if (response.data && response.data.consumerOrderGetResponse) {
                const ordersData = response.data.consumerOrderGetResponse;
    ...
    const handleProfileClick = (order) => {
        if (order.rawOrder && order.rawOrder.farmerProduct && order.rawOrder.farmerProduct.user && order.rawOrder.farmerProduct.user.userID) {
            router.push(`/consumerViewFarmerProfile?userID=${order.rawOrder.farmerProduct.user.userID}`);
        }
    };

    const handleUpdateOrder = (order) => {
        const encodedOrder = encodeURIComponent(JSON.stringify(order.rawOrder));
        router.push(`/consumerUpdateProductDetailsPage?orderDetails=${encodedOrder}&productID=${order.productID}&orderID=${order.orderID}`);
    };
    ...
    const deleteOrder = async () => {
        ...
        try {
            ...
            await axios.put(`http://localhost:8081/api/user/consumer-order-products/${selectedOrder.orderID}/delete`);
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                                <h2 className="text-xl font-poppins-bold text-gray-800">
                                    Consumer Farmer Product Orders ({filteredOrders.length})
                                </h2>
