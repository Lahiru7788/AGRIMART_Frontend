...
const ShoppingCart: React.FC = () => {
    ...
    const fetchCartItems = async () => {
        try {
            ...
            const [farmerResponse, seedsResponse] = await Promise.all([
                fetch(`http://localhost:8081/api/user/viewConsumerOrdersByConsumerID/${userID}`).then(res => res.json()),
                fetch(`http://localhost:8081/api/user/viewConsumerSeedsOrdersByConsumerID/${userID}`).then(res => res.json())
            ]);

            // Filter cart items based on criteria
            const farmerCartItems = farmerResponse.consumerOrderGetResponse?.filter((order: ConsumerOrder) =>
                order.active && order.addedToCart && !order.removedFromCart && order.confirmed && !order.rejected && !order.paid
            ) || [];

            const seedsCartItems = seedsResponse.consumerSeedsOrderGetResponse?.filter((order: ConsumerOrder) =>
                order.active && order.addedToCart && !order.removedFromCart && order.confirmed && !order.rejected && !order.paid
            ) || [];
            ...
            const processedSeedsItems = await Promise.all(
                seedsCartItems.map(async (item: ConsumerOrder) => {
                    ...
                    if (productID) {
                        try {
                            const imageResponse = await fetch(`http://localhost:8081/api/user/viewSAndFProductImage?productID=${productID}`);
                            if (imageResponse.ok) {
                                const blob = await imageResponse.blob();
                                imageUrl = URL.createObjectURL(blob);
                            }
    ...
    const removeFromCart = async (orderID: number, isFarmerProduct: boolean) => {
        try {
            ...
            const apiUrl = isFarmerProduct
                ? `http://localhost:8081/api/user/consumer-removedFromCart/${orderID}/confirm`
                : `http://localhost:8081/api/user/consumerSeeds-removedFromCart/${orderID}/confirm`;
    ...
    const handleCheckout = () => {
        ...
        // Extract all order IDs from cart items with proper validation
        const orderIDs = cartItems
            .filter(item => item.orderID) // Filter out any items without orderID
            .map(item => item.orderID.toString()); // Convert to string and ensure it's valid
        ...
        if (orderIDs.length === 0) {
            console.error('No valid order IDs found in cart items');
            alert('Invalid cart data. Please refresh the page and try again.');
            return;
        }

        // Create URL with order IDs as query parameters
        const orderIDsParam = orderIDs.join(',');

        // Additional validation before navigation
        if (!orderIDsParam || orderIDsParam === '') {
            console.error('Empty orderIDs parameter');
            alert('Unable to process checkout. Please refresh and try again.');
            return;
        }

        console.log('Redirecting to billing with orderIDs:', orderIDsParam);

        // Navigate to billing page with order IDs
        router.push(`/consumerAddBillingDetails?orderIDs=${encodeURIComponent(orderIDsParam)}`);
    };

    const handleContinueShopping = () => {
        router.push('/consumerViewProducts');
    };
                                            ...
                                            <div className="flex items-center space-x-4 flex-1">
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <p className="text-sm text-gray-500 mb-2" style={{ fontFamily: 'Poppins' }}>
                                                        Order #{item.orderID} | Product #{item.isFarmerProduct ? item.farmerProduct?.productID : item.sfProduct?.productID}
                                                    </p>
