...
const CourseCard = ({ course, onPurchaseSuccess }) => {
    ...
    const handleViewTrainerProfile = () => {
        if (course.user && course.user.userID) {
            router.push(`/consumerViewTrainerProfile?userID=${course.user.userID}`);
        }
    };

    // Handle view chapters click - redirect to chapters page
    const handleViewChaptersClick = () => {
        router.push(`/consumerViewCourseChapters?courseID=${course.courseID}`);
    };
    ...
    const handlePurchase = async () => {
        ...
        try {
            ...
            await axios.post(
                `http://localhost:8081/api/user/consumerCourseOrders`,
                purchaseData,
                {
