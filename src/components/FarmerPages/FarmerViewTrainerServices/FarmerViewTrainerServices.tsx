...
const HiringCard = ({ hiring, onUpdateClick }) => {
    ...
    const handleHireNow = async () => {
        try {
            ...
            const response = await axios.post(
                'http://localhost:8081/api/user/farmerHire',
                hireData,
    ...
    const checkConsumerHireStatus = async () => {
        try {
            ...
            const response = await axios.get(
                `http://localhost:8081/api/user/viewFarmerHire/${hiring.hireID}`,
                { withCredentials: true }
            );

            console.log('Consumer hire status response:', response.data); // Debug log

            const consumerHires = response.data.farmerHireGetResponse || [];
    ...
    const handleProfileClick = () => {
        ...
        router.push(`/farmerViewTrainerProfile?userID=${hiring.user.userID}`);
    };
