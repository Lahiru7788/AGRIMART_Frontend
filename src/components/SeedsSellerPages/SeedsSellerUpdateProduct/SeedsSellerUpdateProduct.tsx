...
const ProductCard = ({ product, onUpdateClick }) => {
    ...
    const handleDelete = async () => {
        try {
            await axios.put(
                `http://localhost:8081/api/user/sAndF-product/${product.productID}/delete`,
                {},
...
const UpdateProductForm = ({ product, onCancel, onSubmit }) => {
    ...
    const fetchOfferDetails = async () => {
        try {
            ...
            if (response.data.sfOfferGetResponse &&
                response.data.sfOfferGetResponse.length > 0) {
                const offerDetails = response.data.sfOfferGetResponse[0];
                setOfferFormData({
    ...
    const handleSubmitProductUpdate = async () => {
        try {
            ...
            await axios.post(
                `http://localhost:8081/api/user/seedsAndFertilizerProduct`,
                updatedData,
                { withCredentials: true }
            );
    ...
    const handleImageUpdate = async () => {
        ...
        try {
            ...
            await axios.post(
                `http://localhost:8081/api/user/seedsAndFertilizerProductImage`,
                imageFormData,
                {
                    headers: {
    ...
    const handleSubmitOfferUpdate = async () => {
        try {
            ...
            const endpoint = product.hasOffer ?
                `http://localhost:8081/api/user/sAndFProductOffers` :
                `http://localhost:8081/api/user/sAndFProductOffers`;
...
const ProductList = () => {
    ...
    const [categories, setCategories] = useState(["Fertilizer", "Seeds"]);
