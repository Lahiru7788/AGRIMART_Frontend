...
const OrderNotificationsTable = () => {
    ...
    const fetchOrders = async () => {
        try {
            ...
            const apiEndpoints = [
                `http://localhost:8081/api/user/viewConsumerOrdersBySeedsSellerID/${userID}`,
                `http://localhost:8081/api/user/viewFarmerOrdersBySeedsSellerID/${userID}`,
                `http://localhost:8081/api/user/viewSupermarketOrdersBySeedsSellerID/${userID}`,
            ];
            ...
            responses.forEach((response, index) => {
                if (response.status === 'fulfilled' && response.value.data) {
                    ...
                    const responseKeys = [
                        ...
                        'farmerSeedsOrderGetResponse',
                        'supermarketSeedsOrderGetResponse',
                    ];
                    const orderTypes = [
                        'Consumer',
                        'Farmer',
                    ...
                    if (orders && Array.isArray(orders)) {
                        orders.forEach(order => {
                            const farmerUserID = order.sfProduct?.user?.userID;
                            ...
                            if (farmerUserID && farmerUserID.toString() === userID.toString() &&
                                ...
                                allOrders.push({
                                    ...
                                    category: order.sfProduct?.productCategory || order.productCategory || 'N/A',
    ...
    const handleProfileClick = (order) => {
        if (order.rawOrder && order.rawOrder.user && order.rawOrder.user.userID) {
            router.push(`/farmerViewCustomerProfile?userID=${order.rawOrder.user.userID}`);
        }
    };
    ...
    const confirmOrder = async () => {
        ...
        try {
            ...
            switch (selectedOrder.type) {
                case 'Consumer':
                    apiUrl = `http://localhost:8081/api/user/consumer-seedsOrder-products/${selectedOrder.orderID}/confirm`;
                    break;
                case 'Farmer':
                    apiUrl = `http://localhost:8081/api/user/farmer-seedsOrder-products/${selectedOrder.orderID}/confirm`;
                    break;
                case 'Supermarket':
                    apiUrl = `http://localhost:8081/api/user/supermarket-seedsOrder-products/${selectedOrder.orderID}/confirm`;
                    break;
                default:
    ...
    const rejectOrder = async () => {
        ...
        try {
            ...
            switch (selectedOrder.type) {
                case 'Consumer':
                    apiUrl = `http://localhost:8081/api/user/consumer-seedsOrder-products/${selectedOrder.orderID}/reject`;
                    break;
                case 'Farmer':
                    apiUrl = `http://localhost:8081/api/user/farmer-seedsOrder-products/${selectedOrder.orderID}/reject`;
                    break;
                case 'Supermarket':
                    apiUrl = `http://localhost:8081/api/user/supermarket-seedsOrder-products/${selectedOrder.orderID}/reject`;
                    break;
                default:
                    throw new Error('Invalid order type');
            }
    ...
    const getTypeIcon = (type) => {
        ...
        switch (type) {
            ...
            case 'Farmer': return <Package {...iconProps} />;
