...
const ProductDetailsPage = () => {
    ...
    const orderID = searchParams.get('orderID'); // Get orderID from URL params
        ...
        try {
            ...
            try {
                const imageResponse = await axios.get(
                    `http://localhost:8081/api/user/viewFarmerProductImage?productID=${productID}`,
                    { responseType: "blob" }
                );
            ...
            try {
                const offerResponse = await axios.get(
                    `http://localhost:8081/api/user/viewFarmerOffersByProductId/${productID}`
                );

                if (offerResponse.data.farmerOfferGetResponse &&
                    offerResponse.data.farmerOfferGetResponse.length > 0) {
                    productWithImage.hasOffer = true;
                    setOfferDetails(offerResponse.data.farmerOfferGetResponse[0]);
                }
            } catch (error) {
    ...
    const checkPendingOrder = async () => {
        try {
            ...
            const response = await axios.get(
                `http://localhost:8081/api/user/viewTrainerOrdersByTrainerID/${consumerUserID}`,
                { withCredentials: true }
            );

            if (response.status === 200 && response.data.trainerOrderGetResponse) {
                const allOrders = response.data.trainerOrderGetResponse;
                ...
                const ordersForThisProduct = allOrders.filter(order =>
                    order.farmerProduct?.productID.toString() === productID
                );
    ...
    const handleUpdateOrder = async () => {
        if (quantity <= 0) {
            toast.error("Please select a valid quantity before updating order", {
                position: "top-right",
                autoClose: 3000,
            });
            return;
        ...
        try {
            ...
            const response = await axios.post(
                'http://localhost:8081/api/user/trainerOrderProducts',
                orderData,
                {
                    headers: {
            ...
            if (response.status === 200 || response.status === 201) {
                toast.success(`Order updated successfully! ${requiredQuantityKg}kg of ${product.productName}`, {
                    position: "top-right",
                    autoClose: 4000,
                });
    ...
    const handleBackToOrderHistory = () => {
        router.push('/trainerOrderHistory');
    };
    ...
    return (
        <div className="container mx-auto p-4 mt-[80px] w-[1060px]">
            ...
            <div className="bg-[#88C34E] rounded-t-xl p-4 mb-0">
                <h1 className="text-white font-poppins-bold text-2xl text-center">Update Trainer Order</h1>
            </div>

            <div className={`bg-white rounded-b-xl p-4 ${containerShadowClass}`}>
                {/* Pending Order Alert */}
                {hasPendingOrder && (
                ...
                <div className="flex flex-col md:flex-row">
                    ...
                    <div className="md:w-1/2 pr-4">
                        ...
                        {/* Update Order Button - Full Width */}
                        <button
                            className={`w-full ${isOrdering
